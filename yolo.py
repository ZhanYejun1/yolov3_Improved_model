...
class YOLO(object):
    _defaults = {
        "model_path"        : 'logs/0400-0.3959.pth',
    ...
    def detect_image(self, image):
        ...
        with torch.no_grad():
            ...
            output_list = []
            for i in range(3):
                output_list.append(self.yolo_decodes[i](outputs[i])) # 通过解码操作获得三个特征层
        ...
        thickness = max((np.shape(image)[0] + np.shape(image)[1]) // self.model_image_size[0], 1) #定义框的宽度

        for i, c in enumerate(top_label):
            predicted_class = self.class_names[c]
            score = top_conf[i] #类的得分
