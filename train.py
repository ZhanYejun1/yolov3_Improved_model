...
def fit_ont_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    dict_loss[epoch+1] = '%.4f'%(total_loss/(epoch_size+1))
    if(((epoch+1)%5 == 0) or (epoch ==0)):
        print('Saving state, iter:', str(epoch+1))
        torch.save(model.state_dict(), 'logs/%s-%.4f.pth'%(str(epoch+1).zfill(4),total_loss/(epoch_size+1)))
...
if __name__ == "__main__":
    ...
    #------------------------------------------------------#
    #   迁移学习，载入预训练权重，加快模型训练效率
    #------------------------------------------------------#
    model_path = "model_data/voc_pth.pth"
    ...
    #     Freeze_Epoch = 50
    #
    #     optimizer = optim.Adam(net.parameters(),lr) # 模型优化器
    #     lr_scheduler = optim.lr_scheduler.StepLR(optimizer,step_size=1,gamma=0.95) # 学习率
    #
    #     # 生成训练数据和验证数据
    #     if Use_Data_Loader:
    ...
    if True:
        ...
        for epoch in range(Freeze_Epoch,Unfreeze_Epoch):
            ...
            lr_scheduler.step()
    ftrain = open(os.path.join('logs/','loss.txt'), 'w')
    length_dict = len(dict_loss)
    for i in range(length_dict):
        name= str(i+1).zfill(4)+'-'+(str(dict_loss[i+1]))
        ftrain.write(name)
        ftrain.write('\n')
    ftrain.close()